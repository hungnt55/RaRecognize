...
class biSPOT:
    ...
    def initialize(self, sides, verbose = True):
        """
        Run the calibration (initialization) step
        ...
        S = np.sort(self.init_data)     # we sort X to get the empirical quantile
        self.init_threshold['up'] = S[int(0.8*n_init)] # t is fixed for the whole algorithm
        self.init_threshold['down'] = S[int(0.2*n_init)] # t is fixed for the whole algorithm
        ...
        if (len(self.peaks['down']) <= 1):
          self.extreme_quantile['down'] = self.init_threshold['down']
          return
    ...
    def _rootsFinder(fun,jac,bounds,npoints,method):
        ...
        if method == 'regular':
            step = (bounds[1]-bounds[0])/(npoints+1)
            print(bounds[1], bounds[0], npoints, step)
            X0 = np.arange(bounds[0]+step,bounds[1],step)
        elif method == 'random':
            X0 = np.random.uniform(bounds[0],bounds[1],npoints)
