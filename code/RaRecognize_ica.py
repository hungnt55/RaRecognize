...
def run_test(features, labels, train_classes, test_classes, train_index, test_index):
    ...
    parameters = {'l1':np.append(np.logspace(-2,2,5), 0), 'l2':np.append(np.logspace(-3,1,5), 0)}
    ...
    np.savetxt('test_results/' +sys.argv[1] + '_ica_joint_seen.out', y_pred_score)
    ...
    for classk in range(len(test_classes)):
            ...
            np.savetxt('test_results/' + sys.argv[1] + '_ica_joint_unseen_' + str(classk)+ '.out', y_pred_ri_score)
    ...
    pickle.dump(model, open('test_results/trained_model_' + sys.argv[1] + '_ica_joint' + '.m', 'wb'))
